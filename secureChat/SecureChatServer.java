...
public class SecureChatServer {
	...
	// the set of Data output streams
	private static HashSet<DataOutputStream> clientDataWriters = new HashSet<DataOutputStream>();
		...
		public void run() {
			try {
				...
				while (true) {
					...
					// ** TODO replace this code with login process
				...
				while (true) {
					...
					if (input != null) {
						...
						// ** crypto check the message here
						
						// if message is ok broadcast the byte array to the other clients								
						for (DataOutputStream dWriter : clientDataWriters) {
							dWriter.writeInt(message.length); // write length of the message
